...
export default function ExpenseDetailModal({
  ...
  function getWhatsAppExpenseDetails() {
    ...
    lines.push(
      `*Total Bill Amount:* ${formatCurrency(Number(expense.total_amount))}`
    );
    ...
    if (Array.isArray(expense.paid_by) && expense.paid_by.length > 0) {
      expense.paid_by.forEach((p: any) => {
        lines.push(
          `- ${peopleMap[p.personId] || "Unknown"}: ${formatCurrency(
            Number(p.amount)
          )}`
        );
      });
    ...
    if (expense.celebration_contribution) {
      ...
      lines.push(
        `- Contributed by: ${
          peopleMap[expense.celebration_contribution.personId] || "Unknown"
        }`
      );
      lines.push(
        `- Amount: ${formatCurrency(expense.celebration_contribution.amount)}`
      );
    }
    lines.push("");
    lines.push(
      `*Net Amount For Splitting:* ${formatCurrency(
        Math.max(
          0,
          Number(expense.total_amount) -
            (expense.celebration_contribution
              ? Number(expense.celebration_contribution.amount)
              : 0)
        )
      )}`
    );
    ...
    if (
      ...
      Object.entries(itemwiseBreakdownForDisplay).forEach(
        ([personId, data]: any) => {
          lines.push(`\n*${peopleMap[personId] || "Unknown"}*`);
          data.items.forEach((item: any) => {
            lines.push(
              `  - _${item.itemName}_: Original: ${formatCurrency(
                item.originalItemPrice
              )}, Their Share: ${formatCurrency(
                item.shareForPerson
              )} (shared by ${item.sharedByCount})`
            );
          });
          lines.push(
            `  _Total item share:_ *${formatCurrency(
              data.totalShareOfAdjustedItems
            )}*`
          );
        }
      );
      ...
      if (expense.items) {
        ...
        sortedCategoryNames.forEach((catName) => {
          ...
          itemsByCategory[catName].forEach((item) => {
            ...
            lines.push(
              `- _${item.name}_: ${formatCurrency(
                item.price
              )} (shared by: ${sortedSharedBy
                .map((id: string) => peopleMap[id] || "Unknown")
                .join(", ")})`
            );
          });
        });
    ...
    if (Array.isArray(expense.shares) && expense.shares.length > 0) {
      expense.shares.forEach((s: any) => {
        lines.push(
          `- ${peopleMap[s.personId] || "Unknown"}: ${formatCurrency(
            Number(s.amount)
          )}`
        );
      });
  ...
  const handleCopy = async () => {
    try {
      ...
      toast({
        title: "Copied!",
        description: "Expense details copied for WhatsApp.",
        variant: "default",
      });
    } catch (e) {
      toast({
