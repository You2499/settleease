...
export function generatePersonalReportPDF({
  ...
  const expenseCards = personExpenses.map((expense) => {
    ...
    people.forEach(otherPerson => {
      ...
      const otherPaidAmount = expense.paid_by?.find(p => p.personId === otherPerson.id)?.amount || 0;
      ...
      let netEffect = 0;
      if (Number(myPaidAmount) > 0 && otherTotalShare > 0) netEffect += otherTotalShare;
      if (Number(otherPaidAmount) > 0 && myTotalShare > 0) netEffect -= myTotalShare;
  ...
  // Get settlements involving this person
  ...
  // Net position = sum of all final balances (already includes settlements)
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .your-summary-item { display: flex; gap: 8px; }
    .your-summary-item .label { color: #666; }
    .your-summary-item .value { font-weight: 600; }
    .your-summary-item .value.positive { color: #2e7d32; }
    .your-summary-item .value.negative { color: #c62828; }
    .your-summary-item .value.money { font-family: 'Geist Mono', monospace; }
    
    .net-effect-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 8px; }
    ...
    .net-effect-item .name { font-weight: 500; margin-bottom: 4px; font-size: 11px; }
    .net-effect-item .amount { font-family: 'Geist Mono', monospace; font-size: 13px; font-weight: 600; }
    .net-effect-item .amount.positive { color: #2e7d32; }
    .net-effect-item .amount.negative { color: #c62828; }
    .net-effect-item .label { font-size: 9px; color: #666; text-transform: uppercase; margin-top: 2px; }
    
    .no-effect { color: #999; font-style: italic; text-align: center; padding: 12px; }
    ...
    .settlement-parties { display: flex; align-items: center; gap: 12px; }
    .settlement-arrow { color: #388E3C; font-size: 18px; }
    .settlement-card.outgoing .settlement-arrow { color: #c62828; }
    .settlement-name { font-weight: 600; color: #1a1a2e; }
    .settlement-amount { font-size: 18px; font-weight: 700; font-family: 'Geist Mono', monospace; }
    .settlement-amount.positive { color: #2e7d32; }
    .settlement-amount.negative { color: #c62828; }
    .settlement-meta { font-size: 10px; color: #666; margin-top: 4px; text-align: right; }
    
    .no-data-box { background: #f5f5f5; border-radius: 8px; padding: 20px; text-align: center; color: #666; }
    ...
    .balance-name { font-weight: 600; font-size: 14px; color: #1a1a2e; margin-bottom: 6px; }
    .balance-amount { font-size: 22px; font-weight: 700; font-family: 'Geist Mono', monospace; }
    .balance-card.positive .balance-amount { color: #2e7d32; }
    .balance-card.negative .balance-amount { color: #c62828; }
    .balance-label { font-size: 10px; color: #666; text-transform: uppercase; margin-top: 4px; }
